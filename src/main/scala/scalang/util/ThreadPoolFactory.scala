...
class DefaultThreadPoolFactory extends ThreadPoolFactory {
  ...
  **/
  ...
  lazy val executorPool = new OrderedMemoryAwareThreadPoolExecutor(max_threads, max_memory, max_memory, 1000, TimeUnit.SECONDS, new NamedThreadFactory("executor", 1))
  ...
  def createBatchExecutor(name : String, reentrant : Boolean) : BatchExecutor = {
    if (reentrant) {
      ...
      val pool = new BatchPoolExecutor("scalang", name, max_threads, max_threads, 60l, TimeUnit.SECONDS, queue, new NamedThreadFactory(name, 1))
      pool
    } else {
      batchExecutor
    }
